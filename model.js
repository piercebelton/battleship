var SHIP = 1; // Global constant
var rowCol = 10; // The number of rows/columns
var numTorpedoes = 0; // The number of torpedoes that have been fired
var maxTorpedoes = 50; //The Maximum torpedoes before you lose
var numShips = 0; // The number of ships in the game
var numHits = 0;
var maxHits = 0;
var arrayOfShips = [];
var shipObject = {};
...
function placeShipOfLength(length) {
  ...
  while (attemptsLeft > 0) {
  	...
  	if (mayPlaceShip(x, y, length, placeHorizontal)) {
        ...
        numShips ++;
        maxHits ++;
        shipCoords = shipCoords.concat([x + "," + y]);
        // arrayOfShips = arrayOfShips.concat(y)
    ...
    shipObject = {length, placeHorizontal, shipCoords};
    arrayOfShips = arrayOfShips.concat(shipObject);
    console.log(arrayOfShips);
  	attemptsLeft--;
  }
}

function placeShipOfLengthAndOrientation(length, isHorizontal) {
  var attemptsLeft = 100; // Make sure we don't loop infinitely
  shipCoords = [];
  while (attemptsLeft > 0) {
		var x = Math.floor(Math.random() * rowCol);
  	var y = Math.floor(Math.random() * rowCol);
  	if (mayPlaceShip(x, y, length, isHorizontal)) {
  		var stop = (isHorizontal ? x : y) + length;
        ...
        // arrayOfShips = arrayOfShips.concat(y)
  			...
  			// Uncomment below to step through and see ships being added in real time.
   			// showShips();
    ...
    shipObject = {length, isHorizontal, shipCoords};
    arrayOfShips = arrayOfShips.concat(shipObject);
    console.log(arrayOfShips);
  	attemptsLeft--;
  }
